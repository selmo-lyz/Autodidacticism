    ...
    void train(vector<vector<double>> x_train,
        ...
        for (int time = 0; time < epochs && keep; time++) {
            ...
            if (!converge) {
                ...
                // 計算經歷一個 epoch 後，比對 training data 的準確率
                cout << time << ":\n";

                keep = false;
                double acc = test(x_train, y_train);
    ...
    void output_weight(void) {
        ...
        for (int row = 0; row < topo.size(); row++) {
            for (int col = 0; col < topo[row].size(); col++) {
                ...
                fout << topo[row][col].get_output() << endl;
            }
        }
        fout.close();
    }
    ...
    void input_weight(void) {
        ...
        for (int row = 0; row < topo.size(); row++) {
            for (int col = 0; col < topo[row].size(); col++) {
                ...
                getline(fin, str, '\n');
                topo[row][col].set_output(atof(str.c_str()));
...
int main(int argc, char *argv[]) {
     ...
     * max: weight 隨機值的最大值
     */
    Topology2D topo2D(10, 10, col, -1, 1);
    /*
     * x_train: training_data
     * epochs: 最多執行次數
     * radius: 鄰近半徑
     ...
     * converge: 使訓練到收斂爲止
     */
    topo2D.train(training_data, label, 100, 5, 0.75, 2, 0.75, true);
